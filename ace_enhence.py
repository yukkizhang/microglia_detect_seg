...
def zmIce(I, ratio=4, radius=300):                     #常规的ACE实现  
    ...
    zh,zw = [0]*radius + [x for x in range(height)] + [height-1]*radius, [0]*radius + [y for y in range(width)]  + [width -1]*radius  
    Z = I[np.ix_(zh, zw)]  
...
if __name__ == '__main__': 
    # (/255.0)*255 
    m = zmIceColor(cv2.imread('microglia_cut1.png')/255.0)*255 
    # img = cv2.imread('microglia_cut1.png') 
    # m = zmIceFast(img,ratio=4,radius=3)
    # m = zmIceColor(img)
